...
export default function StandingsPage() {
  ...
  const loadStandings = useCallback(async () => {
    ...
    setLoading(true);
    try { setStandings(await fetchStandings(selectedId)); }
      ...
      {!selectedId ? (
        ...
        <Card>
          ...
          <CardBody className="p-0">
            {standings.map((s, i) => (
              <div key={s.id} className={`grid grid-cols-12 gap-2 items-center px-6 py-3 text-sm border-t border-gray-800/50 ${i === 0 ? 'bg-emerald-500/5' : ''}`}>
                ...
                <div className="col-span-4 min-w-0"><p className="font-semibold truncate">{s.team_name}</p></div>
                ...
                <span className="col-span-1 text-center tabular-nums">{s.goal_difference > 0 ? '+' : ''}{s.goal_difference}</span>
