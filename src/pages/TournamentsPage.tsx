...
import { fetchTournaments, createTournament, updateTournament, deleteTournament } from '../lib/tournamentService';
import type { Tournament } from '../lib/supabaseClient';

const statusTone: Record<Tournament['status'], 'amber' | 'emerald' | 'slate'> = {
  upcoming: 'amber', ongoing: 'emerald', completed: 'slate',
};

const statusFlow: Record<Tournament['status'], Tournament['status'] | null> = {
  upcoming: 'ongoing', ongoing: 'completed', completed: null,
};

const statusActionLabel: Record<Tournament['status'], string> = {
  upcoming: 'Set Ongoing', ongoing: 'Set Completed', completed: 'Completed',
};
...
export default function TournamentsPage() {
  ...
  const handleStatusChange = async (t: Tournament) => {
    ...
    if (!next) return;
    setBusyId(t.id);
    try { await updateTournament(t.id, { status: next }); await load(); }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={() => setShowCreate(true)}><span className="flex items-center gap-2"><Plus size={18} /> Create</span></Button>
      </div>
              ...
              <Card key={t.id}>
                ...
                <CardBody>
                  <div className="flex items-center gap-4 text-sm text-gray-400 mb-4">
                    {t.start_date && <span className="flex items-center gap-1.5"><Calendar size={14} /> {t.start_date}</span>}
                    {t.end_date && <span className="text-gray-600">→ {t.end_date}</span>}
                  </div>
                  <div className="flex items-center gap-2">
                    {next && (
                      <Button size="sm" variant={next === 'ongoing' ? 'primary' : 'outline'} onClick={() => handleStatusChange(t)} disabled={busyId === t.id}>
                    ...
                    <Button size="sm" variant="ghost" onClick={() => handleDelete(t.id)} disabled={busyId === t.id} className="text-red-400 hover:bg-red-500/10">
                      <Trash2 size={14} />
                    </Button>
      ...
      {showCreate && (
        <CreateTournamentModal onClose={() => setShowCreate(false)} onCreated={() => { setShowCreate(false); load(); }} />
      )}
    </div>
  );
}
...
function CreateTournamentModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      await createTournament({ name: name.trim(), description: description.trim(), start_date: startDate || null, end_date: endDate || null });
      onCreated();
    } catch (err) {
  ...
  return (
    <Modal open onClose={onClose} title="Create Tournament">
      <form onSubmit={submit} className="space-y-4">
        <div><Label>Name</Label><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Summer Cup 2026" required /></div>
        <div><Label>Description</Label><Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Tournament description" rows={3} /></div>
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <Button type="submit" disabled={busy}>{busy ? <Loader2 size={18} className="animate-spin" /> : 'Create'}</Button>
        </div>
      </form>
    </Modal>
  );
